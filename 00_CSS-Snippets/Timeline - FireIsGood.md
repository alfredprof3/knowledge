...
body {
  ...
  --timeline-dot-color: white; /* FIX 2: neutral white dot */
...
[data-callout="timeline-fig"] {
  ...
  .callout-title-inner {
    ...
    /* FIX 3: center text on dot — shifts down by (dot_size - text_height) / 2 */
    transform: translateY(calc(var(--timeline-dot-size) / 2 - 50%));
  }
  .callout-icon {
    order: 2;
...
/* FIX 1: Continuous vertical line — remove gap between consecutive items */
div:has(> [data-callout="timeline-fig"]) {
  margin-bottom: 0 !important;
  padding-bottom: 0 !important;
}
div:has(> [data-callout="timeline-fig"]) + div:has(> [data-callout="timeline-fig"]) {
  margin-top: 0 !important;
  padding-top: 0 !important;
}

/* Stacking: small indent before the first timeline item */
div:not(:has(> [data-callout="timeline-fig"])) + div > [data-callout="timeline-fig"] {
  margin-top: var(--p-spacing);
}
/* Stacking: restore normal spacing after the timeline group ends */
div:has(> [data-callout="timeline-fig"]) + div:not(:has(> [data-callout="timeline-fig"])) {
  margin-top: 1rem;
}
